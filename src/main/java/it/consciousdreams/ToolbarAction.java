...
public class ToolbarAction extends AnAction {
    ...
    private void runMavenCommand(Project project) {
        MavenCommandRunner runner = project.getService(MavenCommandRunner.class);
        if (runner == null) {
            Messages.showWarningDialog(project,
                    "Maven integration is not available in this IDE. "
                            + "Use a Shell action with a command such as 'mvn clean install' "
                            + "if Maven is installed on your system.",
                    config.getLabel());
            return;
        }
        runner.run(project, config);
    }
